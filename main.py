...
app = FastAPI(title="Bhaskar Trade Terminal", version="0.2.0")
...
def dhan_headers() -> dict[str, str]:
    if not DHAN_CLIENT_ID or not DHAN_ACCESS_TOKEN:
        raise HTTPException(
            status_code=503,
            detail="Dhan credentials are not configured.",
        )
    return {
...
async def expiries() -> dict[str, Any]:
    if APP_MODE == "demo":
        ...
        return {
            "status": "success",
            "data": [(first + timedelta(days=7 * i)).isoformat() for i in range(4)],
        }
...
def demo_chain() -> dict[str, Any]:
    ...
    for strike in range(23700, 24901, 50):
        ...
        call_delta = max(0.04, min(0.96, 0.5 + moneyness / 950))
        put_delta = -(1 - call_delta)
        gamma = max(0.00025, 0.00145 - abs(moneyness) / 700000)
        time_value = max(7, 108 - abs(moneyness) * 0.14)

        ce_ltp = max(3.0, max(moneyness, 0) + time_value)
        pe_ltp = max(3.0, max(-moneyness, 0) + time_value)
        iv = max(8.5, 9.7 + abs(moneyness) / 350)
